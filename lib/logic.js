...
function addReport(add){
	...
	var patient_promise = getParticipantRegistry('org.healthcare.records.Patient')
        .then(function (userRegistry) {
            patient = userRegistry.get(add.userId)
            if(!patient) {
...
function updateHealthData(update){
	var patient_promise = getParticipantRegistry('org.healthcare.records.Patient')
        .then(function (userRegistry) {
            patient = userRegistry.get(update.userId)
            if(!patient) {
                throw new Error('A patient does not exist.');
            }
